...
func (l *logrusLogger) WithField(key string, val any) Logger {
	return &logrusLogEntry{
		entry: l.logger.WithField(key, val),
	}
}

// WithFields add error to the logging context
...
// WithField adds field to the logging context
func (l *logrusLogEntry) WithField(key string, val any) Logger {
	return &logrusLogEntry{
